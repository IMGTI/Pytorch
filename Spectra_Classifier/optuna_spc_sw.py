...
def data_loader(data_path, constituent, random=False):
    ...
    if data_file in os.listdir():
        ...
        joblib.dump((amp, label, [yes, possible, no]), data_file)

    return amp, label, [yes, possible, no]

def get_weights_transformed_for_sample(sample_weighting_method,
...
def hyp_tune(constituent, num_samples=10, max_num_epochs=10):
    # Data directory
    data_path = 'C:\Datos_ML\Spectra_Classifier\Espectros_analizados\PredMeasure'#'D:/Documents/GitHub/Datos_Espectros/Espectros_analizados/PredMeasure'
    '''
    # Constituent
    ...
    '''
    # Load data
    amp, label, samp_per_cls = data_loader(data_path, constituent, random=True)

    def train_model(trial, amp=amp, label=label, samples_per_cls=samp_per_cls):
